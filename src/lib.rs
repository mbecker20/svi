...
pub fn interpolate_variables(
...
) -> Result<(String, String), VariableInterpolatorError> {
    ...
    let first = open_split
        .pop_front()
        .ok_or(VariableInterpolatorError::SplitEmpty)?;
    ...
    for i in 0..open_split.len() {
        if &open_split[i][0..1] == single_opener {
            ...
            if split2.len() <= 1 {
                return Err(VariableInterpolatorError::NoClosingTags { index: i });
            }
            let variable = split2[0];
            let value = variables
                .get(variable)
                .ok_or(VariableInterpolatorError::NoValueFound { variable: variable.to_string() })?;
...
#[derive(Error, Debug)]
pub enum VariableInterpolatorError {
